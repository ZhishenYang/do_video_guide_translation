...
def masking(lang, sentence, task, n=0):
	...
	if (task == 'color'):
		for token in doc:
			...
			if (is_color_like(token.text) and token.pos_ == 'ADJ'):
				word = "[c]"
...
def load_obtained_video_list(split)->list:
	...
	print("Number of obtained videos: {} in the {}".format(len(obtained_vid_list), dir_path))
...
def main(args):
	...
	final_max_en_tokens_string = ""
	final_max_total_tokens_string = ""
	...
	for i, item in enumerate(results):
		...
		if (int(item[-2]) > final_max_en_tokens):
			...
			final_max_en_tokens_string = item[1]

		if (int(item[-1]) > final_max_total_tokens):
			final_max_total_tokens = int(item[-1])
			final_max_total_tokens_string = item[1]
	...
	info('Maximum tokens without punctuation: {},\n Maximum tokens with punctuation: {}'.format(final_max_en_tokens_string,
																							   final_max_total_tokens_string))	
...
if __name__ == "__main__":
	...
	zh_nlp = spacy.load("zh_core_web_sm")  # Load spacy's Chinese language model, Jieba will be used from tokenization
