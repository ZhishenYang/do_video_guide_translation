...
class ImageFolderDataset(data.Dataset):
    ...
    """
    def __init__(self, root, split, type_of_frame, resize=None, crop=None):
        self.split = split
        #self.root = Path(root).expanduser().resolve() / self.split
        self.root = Path(root)
        # Image list in dataset order
        self.index = self.root / Path(type_of_frame+ '_'+split+"_list.id")
...
if __name__ == '__main__':
    ...
    index_list = []
    index_list_path = Path(root.parent) / Path(args.type_of_frame + '_' + split + '_list.id')

    with index_list_path.open() as f:
        for line in f:
            index_list.append(line)

    print("Number of  images: ", len(index_list),' in ', split)

    # Create dataset
    dataset = ImageFolderDataset(root.parent, split, args.type_of_frame, resize=256, crop=224)
